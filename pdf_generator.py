...
def gerar_laudo_pdf(
    ...
    # BaseDocTemplate + Frame com padding zero: SimpleDocTemplate usa por padrao
    # 6pt de padding interno no frame, o que desalinha Tables (que ignoram esse
    # padding e ficam rente a margem) em relacao a Flowables customizados como
    # SectionHeader (que respeitam o padding) -- daí o desalinhamento entre
    # barras e tabelas. Com padding zero, ambos ficam exatamente na margem.
    doc = BaseDocTemplate(
        output_path,
        pagesize=A4,
    ...
    _frame = Frame(MARGIN_L, MARGIN_B, _AW, H - MARGIN_T - MARGIN_B,
                   leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0,
                   id="normal")
    doc.addPageTemplates([PageTemplate(id="normal", frames=[_frame])])

    def add_spacer(cm_h=0.3):
        story.append(Spacer(1, cm_h * cm))

    def add_section(num, title):
        story.append(SectionHeader(num, title, _AW))
    ...
    if opcoes.get("indice", True):
        ...
        secoes = [
            ("1.", "Identificacao"),
            ("2.", "Documento ART"),
            ("3.", "Resumo Tecnico da Norma NR-10"),
            ("4.", "Criterios de Classificacao"),
            ("5.", "Metodologia de Avaliacao"),
            ("6.", "Avaliacoes Realizadas"),
            ("7.", "Estratificacao dos Dados – Resultados Estatisticos"),
    ...
    if opcoes.get("identificacao", True):
        ...
        # Cliente
        story.append(Paragraph("1.1  Empresa Contratante", styles["subsecao"]))
        cli_data = [
        ...
        if profissionais:
            story.append(Paragraph("1.2  Responsável Técnico pela Elaboração", styles["subsecao"]))
        ...
        if participantes:
            story.append(Paragraph("1.3  Fornecedores de Informações", styles["subsecao"]))
    ...
    if opcoes.get("art", True) and laudo.get("art_pdf_path"):
        ...
        add_spacer(0.3)
        story.append(Paragraph(
            "A seguir apresenta-se o documento de Anotacao de Responsabilidade Tecnica (ART) "
            "referente a este Laudo, devidamente registrado no CREA.",
            styles["body"]))
        add_spacer(0.3)
        ...
        if os.path.exists(art_path):
            try:
                ...
                _art_max_w = _AW
                _art_max_h = H - MARGIN_T - MARGIN_B
    ...
    if opcoes.get("resumo_nr10", True):
        ...
        story.append(Paragraph("3.1  Definicao e Objetivo da Norma", styles["subsecao"]))
        story.append(Paragraph(
        ...
        story.append(Paragraph("3.2  Responsaveis pela Aplicacao", styles["subsecao"]))
        story.append(Paragraph(
        ...
        story.append(Paragraph("3.3  Penalidades Legais pelo Descumprimento", styles["subsecao"]))
        story.append(Paragraph(
        ...
        story.append(Paragraph("3.4  Importancia da Conformidade", styles["subsecao"]))
        story.append(Paragraph(
        ...
        story.append(Paragraph("3.5  Suporte Tecnico", styles["subsecao"]))
        story.append(Paragraph(
    ...
    if opcoes.get("metodologia", True):
        ...
        add_spacer(0.3)
        story.append(Paragraph(
            "A metodologia de avaliacao desenvolvida pela ENGELOGIC baseia-se em uma lista de "
            "verificacao (checklist) estruturada com 10 itens prioritarios, cada um ponderado "
            "conforme seu impacto na seguranca eletrica. Para cada item o avaliador registra "
            "a condicao observada, a resposta obtida, a pontuacao resultante e eventuais "
            "observacoes ou acoes corretivas recomendadas.",
            styles["body"]))
        add_spacer(0.2)

        chk_data = [["Item", "Questao de Avaliacao", "Peso Maximo"]]
        for it in checklist_items:
            max_peso = max((opc.get("peso") or 0.0 for opc in it["opcoes"]), default=1.0)
            chk_data.append([
                str(it["numero"]),
                Paragraph(it["questao"], styles["cell"]),
                "%.0f%%" % (max_peso * 100),
            ])
        chk_t = Table(chk_data, colWidths=[1.2*cm, _AW - 3.5*cm, 2.3*cm])
    ...
    if opcoes.get("resumo", True) and paineis:
        ...
        story.append(Paragraph(
            ...
            styles["body"]))
        add_spacer(0.2)
        story.append(Paragraph("6.1  Situacao Geral dos Paineis", styles["subsecao"]))
    ...
    if opcoes.get("graficos", True) and paineis:
        ...
        try:
            ...
            if len(paineis) > 1:
                story.append(PageBreak())
                story.append(Paragraph(
                    "7.1  Avaliacao de Conformidades",
                    styles["subsecao"]))
                story.append(Paragraph(
        ...
        story.append(PageBreak())
        story.append(Paragraph("7.2  Resumo Estatistico por Item", styles["subsecao"]))
    ...
    if opcoes.get("conclusoes", True):
        ...
        story.append(Paragraph("8.1  Conclusao Tecnica", styles["subsecao"]))
        ...
        story.append(Paragraph("8.2  Recomendacoes Tecnicas", styles["subsecao"]))
        ...
        story.append(Paragraph("8.3  Plano de Acao – Prioridades", styles["subsecao"]))
        pri_data = [
    ...
    if opcoes.get("assinaturas", True):
        ...
        if profissionais:
            story.append(Paragraph(
                "Responsavel Tecnico pela Elaboracao do Laudo:",
                styles["label"]))
            add_spacer(0.8)
            for prof in profissionais:
                nome  = prof.get("nome", "-")
                ...
                story.append(HRFlowable(width=_AW * 0.5, thickness=0.75,
                                        color=HexColor("#1a2332"), hAlign="CENTER",
                                        spaceAfter=8))
                crea_str = "  |  CREA: " + crea if crea else ""
                story.append(Paragraph(
                    "<b>%s</b><br/>%s%s" % (nome, cargo, crea_str),
                    ParagraphStyle("_sig", parent=styles["body"],
